...
- **Remote Git & CLI Operations**: 
  ...
  - If Flutter-specific CLI tasks (like `flutter pub get` or code generation) are needed, the agent must proxy them via SSH:
    ```bash
    ssh nixon@192.168.99.10 "cd /path/to/remote/project && flutter pub get"
    ```